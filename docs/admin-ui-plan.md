...
- [x] 创建 `packages/admin-ui` 包结构和独立 package metadata。
- [x] 从样式规范提取 tokens：背景、表面、边框、文字、状态色、字体、尺寸、动效。
- [x] 将核心样式限定在 `.aui-root` / `aui-*`，不复制当前全局 `* { border-radius: 0 !important }`。
- [x] 建立 `@chaos_team/admin-ui-core`。
- [x] 建立 `@chaos_team/admin-ui-react`。
- [x] 建立 `@chaos_team/admin-ui-vue`。
- [x] 建立 `@chaos_team/admin-ui-svelte` 注册/类型入口。
- [x] 完成第一批组件：
  - [x] Button
  - [x] Card
  - [x] Input
