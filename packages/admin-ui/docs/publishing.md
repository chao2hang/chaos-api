...
@chaos_team/admin-ui-business-react
```
...
```text
.github/workflows/publish-admin-ui.yml
```

触发方式：

```bash
git tag admin-ui-v0.1.0
git push origin admin-ui-v0.1.0
```

或者在 GitHub Actions 中手动执行 `Publish Admin UI packages`。
...
3. 指定 workflow 文件：`publish-admin-ui.yml`。
